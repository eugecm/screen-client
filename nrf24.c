...
void nrf24_init(char* server_addr) {
    ...
    */
    ...
    /* In order to have a fair level or reliability, we set the transmission
       rate to 250kbps (the slowest) and 1 auto retries (ARC=1) with a delay of
       1000us (ARD=1000us). The manual mentions that for 250kbps ARD must always be
       greater than 500us anyway.
     */
    cfg = 0b00110001; /* 0011 = 1000us; 0001 = 1 */
    nrf24_send_command(NRF24_CMD_W_REGISTER | NRF24_REG_SETUP_RETR, &cfg, 1, 0, 0);

    /* Antenna parameters:
       cfg[7] = 0; Disable continuous carrier transmit.
       cfg[6] = 0; Reserved.
       cfg[5] = 1; Set data rate to 250kbps.
       cfg[4] = 0; Reserved.
       cfg[3] = 0; Disable high data rate (we're using 250kbps).
